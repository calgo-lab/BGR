...
from BGR.bgr.soil.modelling.horizon.horizon_models import SimpleHorizonClassifierWithEmbeddingsGeotempsMLPTabMLPHybrid
...
class SimpleHorizonClassificationWithLSTMEmbeddingsGeotempsMLPTabMLPHybridLoss(Experiment):
    def __init__(self, training_args: 'TrainingArgs', target: str, dataprocessor: HorizonDataProcessor):
        ...
        defaults = SimpleHorizonClassificationWithLSTMEmbeddingsGeotempsMLPTabMLPHybridLoss.get_experiment_hyperparameters()
        for key in defaults:
    ...
    def train_and_validate(self,
        ...
        for epoch in range(1, self.training_args.num_epochs + 1):
            ...
            logger.info(
                ...
                --------------------------------
                Training:
                - Hybrid Loss (Cross-Entr./Cosine): {avg_train_loss:.4f}
                ...
                Validation:
                - Hybrid Loss (Cross-Entr./Cosine): {avg_val_loss:.4f}
        ...
        self.trained = True
        return_metrics = {
            'Train Hybrid Loss' : self.train_loss_history[-1],
            'Validation Hybrid Loss' : self.val_loss_history[-1],
    ...
    def test(self,
        ...
        test_metrics = {
            'Test Hybrid Loss': avg_test_loss,
        ...
        logger.info(
                ...
                --------------------------------
                Testing:
                - Hybrid Loss (Cross-Entr./Cosine): {avg_test_loss:.4f}
    ...
    def get_model(self) -> nn.Module:
        return SimpleHorizonClassifierWithEmbeddingsGeotempsMLPTabMLPHybrid(
            ...
            num_classes=len(self.dataprocessor.embeddings_dict['embedding']),
    ...
    def _train_model(self, train_loader, device, model, optimizer):
        ...
        for batch in train_loader_tqdm:
            ...
            padded_pred_horizon_embeddings, padded_pred_logits = model(segments=segments, segments_tabular_features=segments_tabular_features, geo_temp_features=geotemp_features[:, 1:]) # 'index' column not used in model
            ...
            #pred_logits = padded_pred_logits[padded_true_horizon_indices.view(-1) != -1]
            pred_logits = torch.stack([pred for pred, lab in zip(padded_pred_logits.view(-1, padded_pred_logits.size(-1)), padded_true_horizon_indices.view(-1)) if lab != -1]).to(device)
            
            pred_topk_horizon_indices = torch.topk(pred_logits, k=self.topk, dim=1).indices
            ...
            # Compute Hybrid loss
            cosine_loss = self.cosine_loss(pred_horizon_embeddings, true_horizon_embeddings, same_class)
            cross_entropy_loss = self.cross_entropy_loss(pred_logits, true_horizon_indices)
            cosine_weight, cross_entr_weight = self.loss_weights
            train_loss = (cosine_weight * cosine_loss) + (cross_entr_weight * cross_entropy_loss)
    ...
    def _evaluate_model(self, eval_loader, device, model, mode='val'):
        ...
        with torch.no_grad():
            for batch in eval_loader_tqdm:
                ...
                cross_entropy_loss = self.cross_entropy_loss(pred_logits, true_horizon_indices)
                cosine_weight, cross_entr_weight = self.loss_weights
                val_loss = (cosine_weight * cosine_loss) + (cross_entr_weight * cross_entropy_loss)
    ...
    def get_experiment_hyperparameters():
        return {
            'loss_weights' : [1.0, 1.0], # Weights for Cosine, Cross-Entropy according to last validation losses
