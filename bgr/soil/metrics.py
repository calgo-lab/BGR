...
def f1_score(outputs, labels):
    """
    Computes the F1 score for a multi-class problem.

    Args:
        outputs (torch.Tensor): The model's predicted outputs (batch_size, num_classes).
        labels (torch.Tensor): The true labels (batch_size,).
    ...
    fp = (predictions != labels).sum().item()
    fn = (labels != predictions).sum().item()
...
class F1ScoreLoss(nn.Module):
    """
    Computes the F1 score for a multi-class problem as a loss function.
    """
    def __init__(self):
    ...
    def forward(self, outputs, labels):
        ...
        Returns:
            torch.Tensor: The F1 score loss.
        """
        _, predictions = torch.max(outputs, dim=1)
        tp = (predictions == labels).sum().item()
        fp = (predictions != labels).sum().item()
        ...
        f1 = 2 * (precision * recall) / (precision + recall)
        return torch.tensor(1.0, device=outputs.device) - f1